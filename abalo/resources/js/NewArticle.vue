        ...
        <div class="bg-stone-200 h-auto p-4 mx-auto my-4 rounded-md text-black">
            <form @submit.prevent="sendArticle" class="article-form">
                ...
                <div class="article-form__group form-group">
                    ...
                    <input class="article-form__input" type="text" v-model="article.ab_name" id="ab_name" placeholder="Namen des Artikels" required>
                </div>

                <div class="article-form__group form-group">
                    <label class="article-form__label" for="ab_description">Artikel Beschreibung</label>
                    <textarea class="article-form__textarea" v-model="article.ab_description" id="ab_description" placeholder="Beschreibung des Artikels" rows="4" required></textarea>
                </div>

                <div class="article-form__group form-group">
                    <label class="article-form__label" for="ab_price">Preis</label>
                    <input class="article-form__input" type="number" step="0.01" v-model="article.ab_price" id="ab_price" placeholder="Preis des Artikels" required>
                </div>

                <div class="article-form__group form-group">
                    <label class="article-form__label" for="ab_creator_id">Ersteller-ID</label>
                    <input class="article-form__input" type="number" v-model="article.ab_creator_id" id="ab_creator_id" placeholder="ID des Erstellers">
                </div>

                <button class="article-form__submit" type="submit" :disabled="isSubmitting" :class="{ 'disabled': isSubmitting }">Erstellen</button>
            </form>
        </div>
    </div>
</template>
